...
"""
Custom multi-layer implementation of the MD Pool method, based on 
...
- the final multi-layer score is the average of the individual layer scores 
"""
...
class MDPoolMulti(nn.Module):
    """
    Base class for the multi-layer version of the MD Pool OOD detector
    """
    ...
    def predict(self, model, x):
        """
        Compute multi-layer MD score given a trained model and input image x
        ...
        batch_size = len(x)
        batch_scores = []
        hooks = set_forward_hooks(self.activation_dict, getActivation, model, verbose=self.verbose, layer_names=None)
        self.reset()
        ...
        for b in range(batch_size):
            ...
            for key in self.activation_dict:
                ...
                num_feature = test_activ.shape[-1]
                ...
                md = md / num_feature  # normalize by number of features as in FRODO
                total_ood_score += [md]
